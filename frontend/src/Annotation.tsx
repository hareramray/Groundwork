...
import { Check, ChevronLeft, ChevronRight, Copy, Crosshair, Hand, MousePointer2, Plus, Save, Scan, SquareDashed, Trash2, X, ZoomIn, ZoomOut } from 'lucide-react';
import { api } from './api';
...
import { CopyAnnotations } from './CopyAnnotations';
...
export default function Annotation({ image, classes, index, total, onClose, onNavigate, onSaved, notify }: { image: Screenshot; classes: string[]; index: number; total: number; onClose: () => void; onNavigate: (index: number) => void; onSaved: (image: Screenshot) => void; notify: (text: string, error?: boolean) => void }) {
  ...
  useEffect(() => { setDraft(structuredClone(image)); setSelected(image.elements[0]?.id ?? null); setDirty(false); setErrors([]); setZoom(1); setPan([0, 0]); setCopyOpen(false); }, [image.id]);
  ...
  function editBoxCoordinate(coordinate: number, value: number) {
    if (!active || !Number.isFinite(value)) return;
    const bbox = [...active.bbox] as Element['bbox'];
    const gap = coordinate < 2 ? Math.min(0.00001, bbox[coordinate + 2] / 2) : Math.min(0.00001, (1 - bbox[coordinate - 2]) / 2);
    bbox[coordinate] = coordinate < 2 ? clamp(value, 0, bbox[coordinate + 2] - gap) : clamp(value, bbox[coordinate - 2] + gap, 1);
    editElement({ ...active, bbox, click_point: [clamp(active.click_point[0], bbox[0], bbox[2]), clamp(active.click_point[1], bbox[1], bbox[3])] });
  }
  function addCopiedAnnotations(copied: { elements: Element[]; examples: Example[] }) {
    update(value => ({ ...value, elements: [...value.elements, ...copied.elements], examples: [...value.examples, ...copied.examples] }));
    if (copied.elements.length) { setSelected(copied.elements[0].id); setPanel('elements'); }
    else setPanel('instructions');
    setTool('select'); setCopyOpen(false);
    notify(`Added ${copied.elements.length} elements and ${copied.examples.length} instructions to your unsaved draft. Adjust the boxes, then save your changes.`);
  }
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => {
      if (copyOpen) { if ((event.ctrlKey || event.metaKey) && event.key === 's') event.preventDefault(); return; }
  ...
  return <div className="annotation-screen">
    ...
    <div className="annotation-layout"><div className="canvas-column"><div className="canvas-toolbar"><div className="button-group">{([{ value: 'select', icon: MousePointer2, title: 'Select / move (V)' }, { value: 'draw', icon: SquareDashed, title: 'Draw target (D)' }, { value: 'pan', icon: Hand, title: 'Pan (H or hold Space)' }, { value: 'point', icon: Crosshair, title: 'Adjust click point' }] as const).map(t => <button key={t.value} title={t.title} className={`icon-button ${tool === t.value ? 'active' : ''}`} onClick={() => setTool(t.value)} disabled={t.value === 'point' && !active}><t.icon size={18}/></button>)}</div><span className="tool-label">{tool === 'draw' ? 'Drag to draw a target box' : tool === 'point' ? 'Click inside the box to adjust the click point' : tool === 'pan' ? 'Drag to pan your screenshot' : 'Select a box • drag its corners to resize'}</span><div className="button-group"><button className="icon-button" title="Zoom out" onClick={() => zoomTo(zoom / 1.25)}><ZoomOut size={17}/></button><span className="zoom-label">{Math.round(zoom * 100)}%</span><button className="icon-button" title="Zoom in" onClick={() => zoomTo(zoom * 1.25)}><ZoomIn size={17}/></button><button className="icon-button" title="Fit screenshot" onClick={() => { setZoom(1); setPan([0, 0]); }}><Scan size={17}/></button></div></div>
      ...
      <div className="annotation-copy-action"><button className="button full" disabled={saving} onClick={() => { holdingSpace.current = false; drag.current = null; setBox(null); setCopyOpen(true); }}><Copy size={15}/>Copy from image</button><p>Reuse saved boxes and instructions, then resize the boxes to fit this screenshot.</p></div>
    ...
    {copyOpen && <CopyAnnotations destinationId={draft.id} classes={classes} onCopy={addCopiedAnnotations} onClose={() => setCopyOpen(false)}/>}
  </div>;
}
